...
[@sync_brainstorming](../brainstorming/sync_creation.md)

[@sync-example](../../src/syncs/sample.sync.ts)

[@inclusions-and-exclusions](../../src/concepts/Requesting/passthrough.ts)

# implement: I would like to generate a number of sync functions (see sync_creation.md) for all the concepts. The functions listed under exclusions need to be connected to syncs. Please work on the SessionLogging concept. Note that a session does not refer to an authorization session but rather a birding trip session. Your implementation should not change the parameters required of any actions involved in the syncs.
