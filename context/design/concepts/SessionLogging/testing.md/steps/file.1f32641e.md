...
content_id: 1f32641efeb379bb3c134e0cf1976a3c85641cb16165c3e2f98c0ef54bc224ef
---
...
Deno.test("SessionLogging Concept Tests", async (t) => {
  ...
  await t.step(
    ...
    async () => {
      ...
      logAction("endSession (second time - idempotent)", {
        user: userAlice,
        session: sessionAlice3,
      ...
      assertEquals(
        ...
        false,
        "Ending an already inactive session should not error, showing idempotency.",
      );
      assertEquals(
        endSessionAgainResult,
        {},
        "Expected empty success result for idempotent operation.",
      );
      ...
      assertEquals(
        sessionActiveStatus,
        false,
        "Session should still be inactive after idempotent call.",
      );
    },
  );
